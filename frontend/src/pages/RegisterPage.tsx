...
export default function RegisterPage() {
  ...
  const [form, setForm] = useState({
    username: '', email: '', password: '', fullName: '', relationshipToFamily: '', message: '',
  });
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-warm-50 to-stone-100 flex items-center justify-center px-4">
        <div className="bg-white rounded-2xl shadow-lg p-10 max-w-md w-full text-center">
          ...
          <button onClick={() => navigate('/login')} className="mt-6 text-warm-700 hover:text-warm-800 font-medium text-sm">
            Back to Login
          </button>
        </div>
      </div>
      ...
      <div className="w-full max-w-lg">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-8">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input name="fullName" value={form.fullName} onChange={handleChange} required
                  className="w-full border border-stone-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-warm-400 focus:border-warm-400 outline-none" />
              </div>
              <div>
            ...
            <div>
              <label className="block text-sm font-medium text-stone-700 mb-1">Message to Admin <span className="text-stone-400">(optional)</span></label>
              <textarea name="message" value={form.message} onChange={handleChange} rows={3}
                placeholder="Any additional context about your connection to the family..."
                className="w-full border border-stone-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-warm-400 focus:border-warm-400 outline-none resize-none" />
            </div>
          ...
          <p className="text-center text-sm text-stone-500 mt-5">
            Already have access? <Link to="/login" className="text-warm-700 hover:text-warm-800 font-medium">Sign in</Link>
          </p>
        </div>
      </div>
    </div>
